...
Its memory searches by meaning and by keyword at the same time, so a paraphrased question still finds the right fact even when it shares no words with how you first wrote it. Skills are not prompts. They are small, signed programs that rewrite themselves based on how well they actually performed. Pure-compute transforms run in a fuel-bounded, deny-by-default WASM sandbox; the richer script skills (Python and the like) run as signed programs behind an off-by-default shell gate, are refused outright once a run has read untrusted content, and get real network isolation under the Docker backend. Anything the agent does to its own memory or skills is signed, append-only, and reversible in one click, and the dashboard shows it as it happens rather than quietly writing to a folder you are told to review later.

The v0.1 architecture is finished and tested, and 0.2.x builds on it: the reactive core and the signed ledger, hybrid memory, the metered gateway, the signed self-improving skills and their learning loop, the scheduler, the daemon that ties it all together, and the benchmarks. It all builds offline with no network, and passes `cargo test --workspace`.
...
**`crates/engram-skills`, the skill runtime.** Skills are not prompts; they are small, signed programs on **two substrates**, each behind the same capability model. The manifest names the runtime and is signed with the bytes, so a skill can't be silently re-pointed at a different interpreter. (1) **WASM** — pure-compute transforms run in a capability-sandboxed, fuel-bounded host (via `wasmi`, a pure-Rust interpreter chosen for a tiny binary and a deterministic deny-by-default sandbox): a skill receives a host function only if its signed manifest was granted the matching capability, importing anything ungranted fails to link so an over-reaching skill never starts, and a runaway skill traps on fuel exhaustion instead of hanging the core. (2) **Process** — the richer script skills (the seed library is Python) are signed programs that run through the agent's shell backend, so they carry the shell's guarantees rather than WASM's: **off by default** (the `shell` gate must be enabled), **refused outright the instant a run has read untrusted content** (a code-execution taint guard, `skills_runtime.rs:153`), and network-isolated only under the **Docker** backend (`docker run --network none`) — under the local/ssh backend a script inherits the host network, exactly as the `shell` tool does, so true isolation is the Docker backend's job (`skills_runtime.rs:189-197`). A registry versions skills and their recorded runs. On top of both sits the **self-improving learning loop**: a candidate version is replayed against the inputs the skill has actually seen, scored on the skill's own metric, A/B-gated head-to-head against the incumbent, promoted only on a measured win with consent, and one `set_active` away from being reverted. **Egress capabilities (LLM, Net) are revoked automatically for any run that read untrusted input**: the no-egress half of the taint rule.
...
- **`web_search` / `web_fetch`**: real web access, returning readable text (default `web` feature). `web_search` runs a provider cascade — Tavily, then the Brave Search API, then a self-hosted SearXNG when their keys/URL are configured (in `config.json` under `web`, or `TAVILY_API_KEY` / `BRAVE_API_KEY` / `SEARXNG_URL`) — and otherwise falls back to keyless HTML scraping (Brave, then DuckDuckGo html/lite), so it works with **no API key** out of the box. `web_fetch` is a plain fetch behind the SSRF guard.
...
| **Web access** | `web_search` + `web_fetch`, real, no API key required (Tavily / Brave / SearXNG when configured, else keyless HTML scrape) | web search and fetch |
...
| **Self-modifying-skill safety** | 100% of skills signed + capability-gated; pure transforms run WASM (deny-by-default, fuel-bounded), script skills run only behind the off-by-default shell gate, are refused on any untrusted-data run, and are network-isolated under the Docker backend; egress revoked on untrusted-data runs | 0% validation on the live in-agent skill-patch path |
...
- **Voice mode**: API-only, no packaged voice client. The loop itself ships — `POST /v1/voice` (audio → transcribe → run the agent → synthesize → audio) and a multi-turn WebSocket session at `GET /v1/voice/stream` — but it is provider-key-gated (needs STT + TTS via `--features http`), transcribes per turn rather than streaming word-by-word, and there is no bundled microphone UI or standalone voice client. What's missing is the polish and a first-class client, not the loop.
...
| `ENGRAM_MODEL` | `claude-haiku-4-5` | Model id the agent uses for the tool-use loop and delegated subagents. |
| `ENGRAM_VISION_MODEL` | _(`ENGRAM_MODEL`)_ | Override model for the `vision_analyze` tool, if the vision model differs. |
| `ENGRAM_IMAGE_MODEL` | _(provider default)_ | Model for the `image_generate` tool (needs `--features http`). |
| `ENGRAM_TTS_MODEL` | _(provider default)_ | Text-to-speech model for `text_to_speech` and the voice loop (needs `--features http`). |
| `ENGRAM_STT_MODEL` | _(provider default)_ | Speech-to-text model for the voice loop (needs `--features http`). |
| `ENGRAM_TOOLS_SHELL` | _(unset)_ | Set to `1` to enable the `shell` tool. Off by default; always refused once a run is tainted. |
| `ENGRAM_SHELL_BACKEND` | _(local)_ | `docker` runs shell commands in a network-isolated container; `ssh` runs them on a remote host; `singularity` runs them in a Singularity/Apptainer container. Unset runs locally. |
...
| `ENGRAM_SINGULARITY_IMAGE` | _(unset)_ | Image/URI for the `singularity` shell backend. |
| `ENGRAM_WORKDIR` | `<ENGRAM_HOME>/work` | Directory the agent's filesystem tools are confined to (symlink-resolving). |
| `ENGRAM_WORKTREES` | _(unset)_ | Set to `1` on a git workspace to run each task in its own detached `git worktree`, cleaned up on exit, so parallel agents don't clobber each other. |
| `ENGRAM_TASK_TOKEN_BUDGET` | _(config)_ | Per-task token budget for the agent loop; overrides the config value. |
...
| `ENGRAM_ALLOW_INSECURE` | _(unset)_ | **Dangerous.** `1` lets the daemon serve on a non-loopback address with **no** `ENGRAM_API_TOKEN` set — an unauthenticated, self-modifying control plane on the network. Only for a trusted private network; prefer setting a token. |
| `ENGRAM_USE_KEYCHAIN` | _(unset)_ | `1` stores provider keys in the OS keychain instead of the `0600` config file (the macOS keychain otherwise re-prompts for the login password on every launch). |
...
**Skills** runs the seeded library — 40+ signed Process (python3) skills (flight search,
weather, currency, Wikipedia, an email tool, …) alongside the two default WASM skills
`shout` and `ask` (`ask` calls the model through the gateway from inside the sandbox).
The top bar's **"ledger verified · N"** and **today's cost** chips stay live throughout.
...
- **A packaged voice client** and **broader messaging/execution breadth**: the honest
  gaps against Hermes noted above (the voice loop itself ships at `/v1/voice`, gated on a
  speech-capable provider); the MCP client narrows the integration gap in the meantime.
