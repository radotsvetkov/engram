...
1. **On-device embedding model choice and its size/latency budget** — **RESOLVED.** A pure-Rust static (model2vec) embedder: a distilled `[vocab, dim]` matrix read straight from `model.safetensors` with a hand-rolled WordPiece tokenizer, so there is no ONNX runtime and no ML crate — the binary stays tiny and the ~30 MB model is a data directory fetched at deploy time (`scripts/build_embedder.py`), never bundled. Selected with `ENGRAM_EMBED=static`; the trigram-hash embedder remains the zero-config default; the gateway path is the opt-in max. Measured: 100% recall@10 / 0.887 MRR vs trigram's 94% / 0.779. Switching embedders re-embeds existing memories into the new space automatically.
